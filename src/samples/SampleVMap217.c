...
/** Enable/disable VMAP debug dump */
#define VMAP_DEBUG_DUMP 1

/** Dump only first N loops to avoid flooding the console */
#define VMAP_DEBUG_DUMP_LOOPS 20

static int g_dbg_loops_left = VMAP_DEBUG_DUMP_LOOPS;

/**
 * @brief Dump first `words` uint32 values in bdata (raw + ntohl(host)).
 * @param bdata Pointer to uint32 buffer (network byte order data from UEI)
 * @param words Number of uint32 words to dump
 */
static void DumpBdataU32FirstN(const uint32 *bdata, int words)
{
    int i;
    if (!bdata || words <= 0)
    {
        printf("[DBG] bdata is NULL or words<=0\n");
        return;
    }

    printf("[DBG] bdata first %d uint32 words (raw / ntohl):\n", words);
    for (i = 0; i < words; ++i)
    {
        uint32 raw = bdata[i];
        uint32 host = ntohl(raw);
        printf("  [%02d] raw=0x%08X host=%u\n", i, (unsigned int)raw, (unsigned int)host);
    }
}
...
{
    PDNA_PARAMS params = {0, 8, {0, 1, 2, 3, 4, 5, 6, 7}, 100.0, 0, 100, 0, ""};
    ...
    vmapRefreshRate = (params.frequency * params.numChannels) / 1024; // 1000*8/1024=7.8125
    periodns = (long long)floor(1000000000.0 / vmapRefreshRate);      //  128000000ns = 128ms
    ...
    while (!stop)
    {
        ...
        const int req_bytes =
            (int)(params.numSamplesPerChannel * params.numChannels * sizeof(uint32));

#if VMAP_DEBUG_DUMP
        if (g_dbg_loops_left > 0)
        {
            printf("[DBG] DqRtVmapRqInputDataSz: req_bytes=%d act_size=%d (act_words=%d)\n",
                   req_bytes, act_size, act_size / (int)sizeof(uint32));
        }
#endif
        ...
        if (g_dbg_loops_left > 0)
        {
            int data_words = data_size / (int)sizeof(uint32);
            int dump_words = (data_words < 5) ? data_words : 5;

            printf("[DBG] DqRtVmapGetInputData: req_bytes=%d data_size=%d (data_words=%d) avl_size=%d (avl_words=%d)\n",
                   req_bytes,
                   data_size, data_words,
                   avl_size, avl_size / (int)sizeof(uint32));

            // DumpBdataU32FirstN(bdata, dump_words);
